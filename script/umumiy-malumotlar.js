fetch("https://cbu.uz/uz/arkhiv-kursov-valyut/json/")
  ...
  .then((data) => {
    document.querySelector(
      "#Baholovchi-sanasidagi-som-kursi-1"
    ).value = `1 AQSH dollari: ${data[0].Rate} so'm`;
    document.querySelector(
      "#Baholovchi-sanasidagi-som-kursi-2"
    ).value = `1 Yevro: ${data[1].Rate} so'm`;
    document.querySelector(
      "#Baholovchi-sanasidagi-som-kursi-3"
    ).value = `1 Rubl: ${data[2].Rate} so'm`;
  });
...
const baholovchiTashkilotNomi = document.querySelector(
  "#Baholovchi-tashkilot-nomi"
);
const baholovchiTashkilotIsmiSharifi = document.querySelector(
  "#Baholovchi-tashkilot-ismi-sharifi"
);
...
const baholashMaqsadiQiymatTuri = document.querySelector(
  "#Baholash-maqsadiga-qiymat-turi"
);
...
document
  ...
  .addEventListener("click", async function (e) {
    ...
    if (calcMethodSelect.value == "1-usul") {
      ...
      tiklanishQiymatiVar =
        Number(analogCarPriceInput.value) +
        Number(analogCarPriceSetting1Input.value) +
    ...
    if (calcMethodSelect.value == "2-usul") {
      ...
      cPod =
        ((1 - Number(secondMethodInput2.value)) *
          (1 -
            Number(secondMethodInput3.value) -
            Number(secondMethodInput4.value)) *
          Number(secondMethodInput.value)) /
        ((1 + Number(secondMethodInput2.value)) *
          (1 - Number(secondMethodInput3.value)));
      cP =
        cPod *
        (Number(secondMethodInput5.value) / Number(secondMethodInput6.value)) *
        (Number(secondMethodInput7.value) / Number(secondMethodInput8.value));
      cV =
        ((1 - Number(secondMethodInput3.value)) * cP) /
        (1 -
          Number(secondMethodInput3.value) -
          Number(secondMethodInput4.value));
    ...
    if (calcMethodSelect2.value == "3-usul") {
      const elTamirlashQiymati = document.querySelector(
        "[data-tamirlash-qiymat]"
      );
      const elYangiAnalog = document.querySelector("[data-yangi-analog]");

      eskirishHisobi = `${
        (Number(elTamirlashQiymati.value) / Number(elYangiAnalog.value)) * 100
      }`;
    ...
    if (calcMethodSelect2.value == "4-usul") {
      ...
      const arrPricEskiEls = document.querySelectorAll(
        "[data-konst-price-eski]"
      );
      ...
      for (let i = 0; i < arrPricEls.length; i++) {
        eskirishHisobi +=
          Number(arrPricEls[i].value) * Number(arrPricEskiEls[i].value);
      }
    ...
    const jamgarEsk =
      (1 -
        (1 - Number(eskirishHisobi) / 100) *
          ...
          (1 - Number(tashqiEskirishInput.value) / 100)) *
      100;
    console.log(eskirishHisobi);
    ...
    myWindow.document.write(
      await getPopulatedHTML({
        ...
        avtotransportVositasiIshlanganYili:
          avtotransportVositasiIshlanganYili.value,
        // tiklanish hisobi
...
function omegaFunc() {
  if (explCarTypeSelect.value == explCarTypeOption1.value) {
    ...
    omega =
      0.065 * Number(explCarAgeInput.value) +
      0.0032 * Number(explCarDestInput.value);
  } else if (explCarTypeSelect.value == explCarTypeOption10.value) {
    omega =
      0.045 * Number(explCarAgeInput.value) +
      0.002 * Number(explCarDestInput.value);
    ...
    omega =
      0.12 * Number(explCarAgeInput.value) +
      0.001 * Number(explCarDestInput.value);
  }
}
